...
def ui_rand_fn(n, d):
    """Generates random samples from a joint PDF of 'n' uniform independent inputs.
    """
    return np.random.rand([n, d])


def ui_cond_rand_fn(n, dep_idx_set, given_idx_set=None, given_values=None):
    """Generates random samples from a joint PDF of 'n' uniform independent inputs.
    """
    return np.random.rand([n, len(dep_idx_set)])


def sample(problem, N_v, N_o, N_i, joint_pdf_rand_fn=ui_rand_fn,
           cond_pdf_rand_fn=ui_cond_rand_fn):
    ...
    These model inputs are intended to be used with :func:`SALib.analyze.shapley.analyze`.
    ...
    for p in range(0, m):
        ...
        for j in range(1, D - 1):
            ...
            cMx_j = np.reshape(cond_pdf_rand_fn(N_o, cS_j, None, None)
                               [N_o, -1])  # sampled values of the inputs in cS_j
