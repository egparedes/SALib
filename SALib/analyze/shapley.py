...
def analyze_ex(problem, X, Y, N_v, N_o, N_i, print_to_console=False):
    ...
    if print_to_console:
        for i in range(D):
            print("[{}] Shapley: {:.4f}  /  Sobol: {:.4f}  /  "
                  "Total Sobol: {:.4f}".format(problem['names'][i],
                                               results['Sh'][i],
                                               results['S1'][i],
                                               results['ST'][i]))
...
def analyze_rand(problem, X, Y, perms, N_v, N_o, N_i, print_to_console=False):
    """Perform Shapley effects analysis on model outputs.
    ...
    >>> problem = dict(num_vars=3, names=['x1', 'x2', 'x3'])
    >>> N_v, N_o, N_i = 10000, 1000, 3
    >>> X, perms = sample.shapley.sample_rand(problem, m=m, N_v=N_v, N_o=N_o, N_i=N_i)
    >>> Y = Ishigami.evaluate(X)
    >>> Si = analyze.shapley.analyze_rand(problem, X, Y, perms, N_v=N_v, N_o=N_o, N_i=N_i,
                             print_to_console=True)
    """
    ...
    sobol_Y = np.zeros(D)
    # sobol_Y2 = np.zeros(D)
    n_samples_sobol = np.zeros(D)
    tot_sobol_Y = np.zeros(D)
    # tot_sobol_Y2 = np.zeros(D)
    n_samples_total = np.zeros(D)
    ...
    for p in range(m):
        ...
        for j in range(D):
            ...
            if j == 0:
                tot_sobol_Y[cur_perm[j]] = tot_sobol_Y[cur_perm[j]] + C_hat  # Total effect
                # tot_sobol_Y2[cur_perm[j]] = tot_sobol_Y2[cur_perm[j]] + C_hat**2
                n_samples_total[cur_perm[j]] += 1

    shapley_Y = (shapley_Y / m) / var_Y
    # shapley_Y2 = (shapley_Y2 / m) / var_Y**2
    # shapley_YSE = np.sqrt((shapley_Y2 - shapley_Y**2) / m)

    sobol_Y = (sobol_Y / n_samples_sobol) / var_Y  # averaging by number of permutations with j=d-1
    # sobol_Y2 = (sobol_Y2 / n_samples_sobol) / var_Y**2
    # sobol_YSE = np.sqrt((sobol_Y2 - sobol_Y**2) / n_samples_sobol)
    sobol_Y = 1 - sobol_Y
    # sobol_Y2 = 1 - sobol_Y2

    tot_sobol_Y = (tot_sobol_Y / n_samples_total) / var_Y  # averaging by number of permutations with j=1
    # tot_sobol_Y2 = (tot_sobol_Y2 / n_samples_total) / var_Y**2
    # tot_sobol_YSE = np.sqrt((tot_sobol_Y2 - tot_sobol_Y**2) / n_samples_total)
